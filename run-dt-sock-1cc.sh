...
$QEMU \
    ...
    -display none -daemonize -pidfile /tmp/qemu-dt.pid \
    -m 1G \
    -smp cpus=8,sockets=1,clusters=2,cores=4,threads=1 \
    -chardev socket,id=ser0,path=/tmp/qemu-dt.sock,server=on,wait=off,logfile=/tmp/qemu-dt.log \
    -serial chardev:ser0 \
...
echo "QEMU pid: $(cat /tmp/qemu-dt.pid)"
echo "Socket:   /tmp/qemu-dt.sock"
