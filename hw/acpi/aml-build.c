                                           ...
                                           uint32_t priv_num)
{
    ...
    if (priv_num > 0) {
        ...
        for (i = 0; i < priv_num; i++) {
            fprintf(stderr, "[QEMU] %s(): append priv_rsrc[%d]: 0x%x\n",
                ...
                const CPUCaches *CPUCaches)
{
    ...
    for (n = 0; n < cpus->len; n++) {
        ...
        if (cpus->cpus[n].props.socket_id != socket_id) {
            ...
            fprintf(stderr, "[QEMU] %s(): cpu = %d: create phys pkg socket_id = 0x%lx\n",
        ...
        if (CPUCaches) {
             ...
             */
            if (cluster_id == 0) {
                fprintf(stderr, "[QEMU] %s(): create L2 cluster 0: next_level = 0x%x\n",
                        __func__, l3_offset);
                build_cache_structure(table_data, l3_offset,
                                      CPUCaches->l2_cluster1_cache);
            } else {
                fprintf(stderr, "[QEMU] %s(): create L2 cluster 1: next_level = 0x%x\n",
                        __func__, l3_offset);
                build_cache_structure(table_data, l3_offset,
            ...
            fprintf(stderr, "[QEMU] %s(): priv_rsrc[1]: 0x%x\n",
                    __func__, priv_rsrc[1]);
            fprintf(stderr, "[QEMU] %s(): create l1d cache for core\n",
                    __func__);
            ...
            fprintf(stderr, "[QEMU] %s(): priv_rsrc[2]: 0x%x\n",
                    __func__, priv_rsrc[2]);
            fprintf(stderr, "[QEMU] %s(): create l1i cache for core\n",
                    __func__);
            build_cache_structure(table_data, priv_rsrc[0],
                                  CPUCaches->l1i_cache);
            priv_num = 2;
        ...
        if (ms->smp.threads == 1) {
            fprintf(stderr, "[QEMU] %s(): cpu %d: proc 0x%x with %d priv res 0x%x\n",
